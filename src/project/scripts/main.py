...
class Quadrotor():
	def __init__(self):
		...
		self.lamda1 = 0.0005
		self.lamda2 =0.0000000005
		self.lamda3 = 0.0000000000005
		self.lamda4 = 0.0000000000005

		self.k1 = 0.0096
		self.k2 = 0.000000000005
	...
	def smc_control(self, xyz, xyz_dot, rpy, rpy_dot):
		...
		s2 = (dphi) + self.lamda2*(phi - self.phi_d)
		s3 = (dtheta) + self.lamda3*(theta - self.theta_d)
		s4 = (dpsi) + self.lamda4*(psi)

		self.u2 = - ((dtheta*dphi*(self.Iy-self.Iz)) -
					 (self.Ip*self.ohm*dtheta)+(self.lamda2*self.Ix*dphi)+self.k2)*self.saturation(s2)
		self.u3 = - ((dphi*dpsi*(self.Iz-self.Ix)) +
					 (self.Ip*self.ohm*dphi)+(self.lamda3*self.Iy*dtheta)+self.k3)*self.saturation(s3)
		self.u4 = - ((dphi*dtheta*(self.Ix-self.Iy)) +
					 (self.lamda4*self.Iz*dpsi)+self.k4)*self.saturation(s4)
