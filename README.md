...
The conditional statements provide the trajectories between two points based on current time. It returns trajectory start and end point as follows:
 - $t<5s \rightarrow p_{0} - p_{1}$ 
 - $5s<t<20s \rightarrow p_{1} - p_{2}$ 
 - $20s<t<35s \rightarrow p_{2} - p_{3}$ 
 - $35s<t<50s \rightarrow p_{3} - p_{4}$ 
 - $50s<t<65s \rightarrow p_{4} - p_{5}$ 
 - $65s<t<70s \rightarrow p_{5} - p_{6}$ 
...
We are designing the controller with reduced chattering, so we shall be adding a boundary layer of $\gamma$ which is the acceptable tolerance. Thus, assume $u_{r} = K_2 sat(s_2)$. Therefore;
...
for $K_2 > 0$, the resulting system is always negative, thus asymptotically stable. Thus,
...
$$
\begin{equation}
\dot{s_{3}} = \ddot{e} + \lambda_{3}\dot{e}\notag
\end{equation}
$$
...
$$
\begin{equation}
s_{3}\dot{s_{3}} = s_{3}(\dot{\phi}\dot{\psi}\frac{I_{z}-I_{x}}{I_{y}}+\frac{I_{p}}{I_{y}}\Omega \dot{\phi}+\frac{1}{I_{y}}u_{3} - \ddot{\theta_{d}} + \lambda_{3}(\dot{\theta}-\dot{\theta_{d}}))\notag
\end{equation}
$$
...
$$
\begin{equation}
s_{3}\dot{s_{3}} = \frac{s_{3}}{I_{y}}((\dot{\phi}\dot{\psi}(I_{z}-I_{x})+I_{p}\Omega \dot{\phi}+ - \ddot{\theta_{d}} + \lambda_{3}(\dot{\theta}-\dot{\theta_{d}}))+u_{3})\notag
\end{equation}
$$
